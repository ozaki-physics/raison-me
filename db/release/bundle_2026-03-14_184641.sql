-- bundle generated at 2026-03-14_184641
BEGIN;
...
-- >>> START 0002-capital-coin_data.sql >>>
...
-- <<< END 0002-capital-coin_data.sql <<<

-- >>> START 0003-capital-coin_transaction.sql >>>
...
-- <<< END 0003-capital-coin_transaction.sql <<<
...
DO $$ BEGIN
  ...
  ON CONFLICT (filename) DO NOTHING;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE filename = '0002-capital-coin_data.sql' AND sha256 <> '3b61e517e18b5158f9a6bc68b082d72a1bc12e73e713b4e2f47515c54b480a99') THEN
    RAISE EXCEPTION 'Hash mismatch for 0002-capital-coin_data.sql';
  END IF;
  INSERT INTO public.schema_migrations(filename, sha256)
  VALUES ('0002-capital-coin_data.sql', '3b61e517e18b5158f9a6bc68b082d72a1bc12e73e713b4e2f47515c54b480a99')
  ON CONFLICT (filename) DO NOTHING;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE filename = '0003-capital-coin_transaction.sql' AND sha256 <> '1c0441e33d1f5fafbcb8e6c04447e4a6f5f6ea496f69c495f37ed5d5a4474352') THEN
    RAISE EXCEPTION 'Hash mismatch for 0003-capital-coin_transaction.sql';
  END IF;
  INSERT INTO public.schema_migrations(filename, sha256)
  VALUES ('0003-capital-coin_transaction.sql', '1c0441e33d1f5fafbcb8e6c04447e4a6f5f6ea496f69c495f37ed5d5a4474352')
  ON CONFLICT (filename) DO NOTHING;
  ...
  ON CONFLICT (filename) DO NOTHING;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE filename = '0005-info-user.sql' AND sha256 <> 'eb688397faadf0a9fa16e5f75308c461d88f265aa86de318843a9ecd600408a5') THEN
    RAISE EXCEPTION 'Hash mismatch for 0005-info-user.sql';
  END IF;
  INSERT INTO public.schema_migrations(filename, sha256)
  VALUES ('0005-info-user.sql', 'eb688397faadf0a9fa16e5f75308c461d88f265aa86de318843a9ecd600408a5')
  ON CONFLICT (filename) DO NOTHING;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE filename = '0006-info-user_data.sql' AND sha256 <> 'a19608daa9513ae2684761233ceb053e9cbc3168e17605d6c51187bcac5bfb79') THEN
    RAISE EXCEPTION 'Hash mismatch for 0006-info-user_data.sql';
  END IF;
  INSERT INTO public.schema_migrations(filename, sha256)
  VALUES ('0006-info-user_data.sql', 'a19608daa9513ae2684761233ceb053e9cbc3168e17605d6c51187bcac5bfb79')
  ON CONFLICT (filename) DO NOTHING;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE filename = '0007-info-pass.sql' AND sha256 <> 'f1b2eafc3d7f5b3f07bf11385cbff63307fcc127bae11ef8a24301396fe8cf44') THEN
    RAISE EXCEPTION 'Hash mismatch for 0007-info-pass.sql';
  END IF;
  INSERT INTO public.schema_migrations(filename, sha256)
  VALUES ('0007-info-pass.sql', 'f1b2eafc3d7f5b3f07bf11385cbff63307fcc127bae11ef8a24301396fe8cf44')
  ON CONFLICT (filename) DO NOTHING;
  IF EXISTS (SELECT 1 FROM public.schema_migrations WHERE filename = '0008-info-pass_data.sql' AND sha256 <> '03ca08ab81e20fdcb288837e8d0d784f45d6a5fa10ab84ef17fdcf23a4206ca8') THEN
    RAISE EXCEPTION 'Hash mismatch for 0008-info-pass_data.sql';
  END IF;
  INSERT INTO public.schema_migrations(filename, sha256)
  VALUES ('0008-info-pass_data.sql', '03ca08ab81e20fdcb288837e8d0d784f45d6a5fa10ab84ef17fdcf23a4206ca8')
  ON CONFLICT (filename) DO NOTHING;
END; $$;
COMMIT;
